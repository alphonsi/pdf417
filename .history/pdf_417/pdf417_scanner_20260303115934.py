...
def scan_pdf417_barcode(image_path: str, parse_aamva: bool = False) -> Union[str, Dict[str, Any]]:
    """
    Scan and decode a PDF417 barcode from an image file.
    
    Args:
        image_path (str): Path to the image file containing the barcode
        parse_aamva (bool): If True, attempt to parse AAMVA data and return structured data
    
    Returns:
        Union[str, Dict[str, Any]]: Decoded data from the first PDF417 barcode found, 
        or structured AAMVA data if parse_aamva=True, or empty string if none found
    """
    try:
